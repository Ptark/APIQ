...
class HandcraftedAgent(Agent):
    ...
    def calculate_action(self, percept: Tuple[str, str]) -> str:
        ...
        if self.turn == 0:
            ...
            self.sign = 1 if int(percept[1]) == 0 else -1
        environment_switch = {
            0: self.biased_coin_flip,
            1: self.double_coin_flip,
            2: self.slide
        }
        environment_reversed_switch = {
            0: self.biased_coin_flip_reversed,
            1: self.double_coin_flip_reversed,
            2: self.slide_reversed
        }
        if self.sign == 1:
            action = environment_switch.get(self.idx)(percept)
        else:
            action = environment_reversed_switch.get(self.idx)(percept)
        self.turn += 1
        return action
